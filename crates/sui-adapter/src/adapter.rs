...
use sui_types::{
    base_types::*,
    error::{ExecutionError, FunctionNameResolver, NullResolver},
...
>(
    ...
    let module = vm
        ...
        .map_err(|e| convert_vm_error(e, VMResolver{ vm, state_view}))?;
...
>(
    ...
    for (idx, ty) in type_args.iter().enumerate() {
        session
            .load_type(ty)
            .map_err(|e| convert_type_argument_error(idx, e, VMResolver { vm, state_view }))?;
    }
...
>(
    ...
    let mut modules = module_bytes
        ...
        .map_err(|e| convert_vm_error(e, NullResolver))?;
...
>(
    ...
    session
        ...
        .map_err(|e| convert_vm_error(e, VMResolver{ vm: &vm, state_view}))?;
...
fn convert_type_argument_error(
    idx: usize,
    error: VMError,
    vm_resolver: impl FunctionNameResolver,
) -> ExecutionError {
...
struct VMResolver<'vm, M: MoveResolver> {
    vm: &'vm MoveVM,
    state_view: &'vm M,
}

impl<M: MoveResolver> FunctionNameResolver for VMResolver<'_, M> {
    fn resolve(&self, m: &ModuleId, idx: FunctionDefinitionIndex) -> Option<Identifier> {
        let module = self.vm.load_module(m, self.state_view).ok()?;
